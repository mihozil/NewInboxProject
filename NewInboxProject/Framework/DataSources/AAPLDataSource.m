...
- (AAPLLoadableContentStateMachine *)stateMachine
{
    ...
    _stateMachine.delegate = self; // minhnht changed: self -> self.delegate
    return _stateMachine;
}
...
- (void)notifyWillUpdateItems:(NSArray *)newItems {
    AAPL_ASSERT_MAIN_THREAD;
    id<AAPLDataSourceDelegate> delegate = self.delegate;
    if ([delegate respondsToSelector:@selector(dataSource:willUpdateItems:)]) {
        [delegate dataSource:self willUpdateItems:newItems];
    }
}
