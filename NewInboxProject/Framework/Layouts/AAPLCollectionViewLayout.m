...
@property (strong, nonatomic) id<UICollectionViewDelegateFlowLayout> delegateFlowLayout;
// setCollectionView ...
...
- (void)populateSection:(AAPLLayoutSection *)section fromMetrics:(AAPLDataSourceSectionMetrics *)metrics
{
    ...
    for (NSInteger itemIndex = 0; itemIndex < numberOfItemsInSection; ++itemIndex) {
        ...
        if ([self.collectionView.delegate respondsToSelector:@selector(collectionView:layout:sizeForItemAtIndexPath:)]) {
            CGSize itemSize = [(id<UICollectionViewDelegateFlowLayout>)self.collectionView.delegate collectionView:collectionView layout:self sizeForItemAtIndexPath:[NSIndexPath indexPathForItem:itemIndex inSection:0]]; // minhnht changed
            rowHeight = itemSize.height;
            // note: only care to rowHeight
        }
        
        itemInfo.frame = CGRectMake(0, 0, columnWidth, rowHeight);
        itemInfo.hasEstimatedHeight = NO;
//        if (variableRowHeight) {
//            itemInfo.hasEstimatedHeight = NO; // original Yes -> me make into No
//        }
